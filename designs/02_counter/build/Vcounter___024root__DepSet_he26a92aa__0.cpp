...
VL_INLINE_OPT void Vcounter___024root___nba_sequent__TOP__0(Vcounter___024root* vlSelf) {
    ...
    if (vlSelfRef.rst_n) {
        if (vlSelfRef.load) {
            __Vdly__count = vlSelfRef.din;
        } else if (vlSelfRef.en) {
            __Vdly__count = (0xffU & ((IData)(vlSelfRef.up_dn)
                                       ? ((IData)(1U) 
    ...
    vlSelfRef.zero = (0U == (IData)(vlSelfRef.count));
    vlSelfRef.max_val = (0xffU == (IData)(vlSelfRef.count));
}
...
#endif
            VL_FATAL_MT("src/counter.v", 2, "", "NBA region did not converge.");
        }
...
#endif
                VL_FATAL_MT("src/counter.v", 2, "", "Active region did not converge.");
            }
...
void Vcounter___024root___eval_debug_assertions(Vcounter___024root* vlSelf) {
    ...
    if (VL_UNLIKELY(((vlSelfRef.up_dn & 0xfeU)))) {
        Verilated::overWidthError("up_dn");}
}
#endif  // VL_DEBUG
