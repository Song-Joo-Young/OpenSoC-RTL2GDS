...
> 환경 구축부터 GDS 생성까지, 4-bit counter 디자인으로 전체 flow를 단계별로 수행한다.
> 모든 실행은 `training/counter4/` 디렉토리의 번호별 스크립트로 진행한다.
...
각 Part는 `training/counter4/` 안의 번호별 스크립트에 대응:
...
다른 디자인에 적용하려면 `design.cfg`와 `rtl.f`만 수정한다.
...
`training/counter4/src/counter4.v`:
...
```bash
cp -r training/counter4 training/my_design
...
난이도 순서:
1. 8-bit counter (`designs/02_counter/`)
2. ALU (`designs/03_alu/`)
3. 2x2 Systolic Array (`designs/06_systolic/`)
4. PicoRV32 RISC-V (`designs/04_picorv32/`)
5. SoC + SRAM (`designs/05_soc/`)
